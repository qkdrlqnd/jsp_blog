...
CREATE TABLE member ( 
    ...
    PRIMARY KEY (id)               -- 한번 등록된 id는 중복 안됨, 인덱스기능
); 
...
WHERE id='user1'; 
 ...
 ---
   ...
   INSERT INTO member(id, passwd, mname, tel, email, zipcode,  
address1,address2, job, mdate, fname, grade) 
VALUES('admin', '1234', '개발자1', '123-1234', 'admin@mail.com',  
...
ORDER BY mdate DESC; 
 ...
 select * from member
		...
		select id,  mname,  email, mdate, fname, grade, r from (
			select id,  mname, email, mdate, fname, grade, rownum r from (
				select id,  mname, email, mdate, fname, grade from member
				 where id like '%user%'
