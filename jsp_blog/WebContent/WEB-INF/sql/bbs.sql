...
- 서브쿼리를 이용한 레코드 추가 
INSERT INTO bbs(bbsno, name, title, content, passwd, wdate, grpno)  
VALUES((SELECT NVL(MAX(bbsno), 0) + 1 as bbsno FROM bbs), 
'왕눈asd이', '제목', '내용', '123', sysdate,
...
3. 전체 글 목록(S:Total Select List), 최신글 먼저 출력 
   - ASC: 오름 차순, DESC: 내림 차순 
	...
	select  bbsno, wname, title, content, passwd, viewcnt, wdate, grpno, indent, r	from(
...
FROM bbs; 
 ...
 select  bbsno, wname, title,  viewcnt, wdate, grpno, ansnum, indent, r from(                                                                           
 		select bbsno, wname, title, viewcnt, wdate, grpno, indent, ansnum, rownum r	from(                                                                  
